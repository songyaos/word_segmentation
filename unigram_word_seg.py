...
def word_seg(input_line):
    ...
    if len(myheap) ==0 : 
        for i in range(10):
            if i < line_len-1:
                heappush(myheap, make_entry(input_line[0:i+1].encode('utf-8'),0, (i+1)*not_found, None))
    ...
    while myheap:        
        ...
        for t in range(word_max_len):
            i = word_max_len - t -1 #backward lookup
        ...
        if end_index != line_len-1:#if no matching word, push next two words into heap
            if insert_flag == 0:
                ...
                current_entry = make_entry(current_word,end_index+1,top_entry[2]+log10(Pw.missingfn(current_word,2*Pw.N)),end_index)
                myheap.append(current_entry)
                #current_word = input_line[end_index + 1: end_index + 3].encode('utf-8')
                #current_entry = make_entry(current_word,end_index+1,top_entry[2]+3*log10(Pw.missingfn(current_word,2*Pw.N)),end_index)
                #myheap.append(current_entry)
